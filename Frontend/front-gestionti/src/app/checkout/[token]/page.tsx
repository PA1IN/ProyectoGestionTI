...
export default function CheckoutPage({ params }: { params: Promise<{ token: string }> }) {
    ...
    const ProcesarPago = async (datosTarjeta: DatosPagoTarjeta) => {
        try {
            ...
            router.push(`/resultado/exito?comercio=${transaccion?.comercio}&returnUrl=${encodedReturnUrl}&transactionId=${respuesta.transactionId}`);
        } catch (error) {
    ...
    const anulacion = () => {
        const urlRetorno = transaccion?.returnUrl;

        if (urlRetorno)
        {
            router.push(`${urlRetorno}?status=CANCELADOS&token=${token}`);
        } else {
            router.push('/');
        }
    }
        ...
        <div className="min-h-screen bg-gray-50 flex items-center justify-center py-10 px-4">
            <div className="max-w-4xl w-full bg-white rounded-2xl shadow-xl overflow-hidden flex flex-col md:flex-row">
                ...
                <div className="flex-1 p-8">
                    ...
                    />
                    <button onClick={anulacion} className="text-sm text-gray-500 hover:text-gray-700 underline text-center w-full block">
