    ...
    if (isLoading) {
        return (
            <div className="text-center text-sm text-gray-500 py-8 animate-pulse">
                Cargando registros de Conciliaciones...
            </div>
        );
    }
                        ...
                        {historial?.map((registro) => (
                            <tr key={registro.id} className="hover:bg-gray-50 transition-colors">
                                ...
                                <td className="px-6 py-4 flex items-center gap-2">
                                    ...
                                    <span className="truncate max-w-[200px]" title={registro.archivo_id}>
                                        {registro.archivo_id}
                                    </span>
                                </td>
                                <td className="px-6 py-4 font-mono">
                                    {registro.registros_banco}
                                </td>
                                <td className="px-6 py-4">
                                    {registro.discrepancias_encontradas > 0 ? (
                                        <span className="flex items-center gap-1 text-amber-600 bg-amber-50 px-2 py-1 rounded-md text-xs font-bold w-fit">
                                            <AlertCircle className="w-3 h-3"/> {registro.discrepancias_encontradas}
                                        </span>
                                    ) : (
